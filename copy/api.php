<?php
// Ustawienie nagłówków, aby umożliwić komunikację z frontendu (CORS) i określić typ odpowiedzi
...
// --- Logika Biznesowa z mechanizmem fallback ---
try {
    $wasFallbackNeeded = false;
    $matchedJobs = [];

    // KROK 1: Pierwsza próba - znajdź zawody pasujące do profilu i branży
    ...
    // KROK 2: Druga próba (fallback) - jeśli nic nie znaleziono, szukaj tylko po profilu
    ...
    // Ostateczne sprawdzenie - jeśli nadal nic nie ma, to faktycznie brak wyników
    if (empty($matchedJobs)) {
        http_response_code(404);
        echo json_encode(['message' => 'Nie znaleziono pasujących zawodów dla profilu: ' . $mainProfile]);
        exit;
    }

    // KROK 3: Zbierz dodatkowe informacje dla znalezionych zawodów
    $detailedCareerPaths = [];
    ...
    $systemPrompt = "Jesteś 'Kompasem Kariery Płock', eksperckim doradcą zawodowym. Twoim zadaniem jest wygenerowanie spersonalizowanego raportu w formie ciągłego, naturalnego tekstu. Odpowiedź musi być wyłącznie obiektem JSON. Kategorycznie zabronione jest używanie jakichkolwiek list, punktorów czy emoji. Całość musi być spójnym tekstem w każdej sekcji.\n\nStruktura JSON musi być następująca:\n{\n  \"profileTitle\": \"string\",\n  \"profileDescription\": \"string\",\n  \"pathsIntro\": \"string\",\n  \"careerPaths\": [\n    {\n      \"title\": \"string\",\n      \"description\": \"string\",\n      \"qualifications\": [\"string\"],\n      \"employers\": \"string\"\n    }\n  ],\n  \"nextSteps\": [\"string\"]\n}";

    $userPrompt = "Proszę o wygenerowanie raportu kariery.\n\nDane wejściowe:\n- Dominujący profil RIASEC: {$profileCode}\n- Twoje wykształcenie: {$educationLevel}\n- Twój cel edukacyjny: {$futurePath}\n- Preferowana branża: {$preferredIndustry}\n- Mocne strony: " . json_encode($strengths) . "\n- Preferencje środowiska: " . json_encode($preferences) . "\n\nDopasowane ścieżki kariery (wybrane z bazy danych):\n" . json_encode($detailedCareerPaths, JSON_UNESCAPED_UNICODE);
    
    if ($wasFallbackNeeded && $preferredIndustry !== 'Brak') {
        $userPrompt .= "\n\nWAŻNA INFORMACJA DLA AI: Nie udało się znaleźć zawodów, które jednocześnie pasują do profilu użytkownika ('{$mainProfile}') i wybranej przez niego branży ('{$preferredIndustry}'). Dlatego przedstawione powyżej ścieżki kariery bazują głównie na profilu RIASEC. W sekcji 'pathsIntro' delikatnie wyjaśnij użytkownikowi, że przedstawione propozycje są szersze niż jego preferencje branżowe, ale wciąż doskonale pasują do jego osobowości i mocnych stron.";
    }

    $userPrompt .= "\n\nTwoim zadaniem jest wybranie 4 najbardziej pasujących ścieżek z powyższej listy i stworzenie dla nich motywujących opisów oraz wygenerowanie sekcji 'Następne kroki'. Pamiętaj o kategorycznym zakazie używania list i punktorów. Zwróć wynik w wymaganym formacie JSON.";
    ...
    http_response_code(500);
    echo json_encode(['message' => 'Wystąpił błąd serwera podczas generowania raportu: ' . $e->getMessage()]);
}
?>
