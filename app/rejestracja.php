<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($conn->connect_errno) {
            // Bezpieczny komunikat o błędzie połączenia
            ...
            if (empty($nazwa) || empty($haslo) || empty($email)) {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($conn->errno === 1062) {
                        ...
                        $error_message = "Wystąpił błąd podczas rejestracji. Spróbuj ponownie.";
                    }
                }
                $stmt->close();
            }
...
<head>
    ...
    <title>KIERUNEKPLOCK.PL - Rejestracja</title>
...
<header class="bg-brand-blue text-white sticky top-0 z-50 shadow-lg">
    ...
    <main class="flex-grow container mx-auto p-4 flex items-center justify-center">
        <div class="w-full max-w-md">
            <form method="POST" class="bg-white p-8 rounded-2xl shadow-xl space-y-4 fade-in">
                <h1 class="text-3xl font-bold text-center text-brand-blue">Strona Rejestracji</h1>
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" required class="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-blue">
                </div>

                <div>
                    <label for="telefon" class="block text-sm font-medium text-gray-700 mb-1">Telefon (opcjonalnie)</label>
                    <input type="tel" name="telefon" id="telefon" class="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-blue">
                </div>

                <input type="submit" value="Zarejestruj się" class="w-full bg-brand-blue text-white py-3 px-5 rounded-lg font-semibold hover:opacity-90 transition-opacity cursor-pointer mt-2">

                <div class="text-center space-y-2 pt-2">
                    <a href="login.php" class="text-brand-blue hover:underline font-medium">Masz już konto? Zaloguj się</a>
...
<footer class="bg-brand-blue text-white p-4" style="background-color: #3b5998;">
    ...
    <script>
        ...
        if(mobileMenuOpenBtn) mobileMenuOpenBtn.addEventListener('click', () => mobileMenu.classList.remove('translate-x-full'));
        if(mobileMenuCloseBtn) mobileMenuCloseBtn.addEventListener('click', () => mobileMenu.classList.add('translate-x-full'));
    </script>
</body>
</html>
