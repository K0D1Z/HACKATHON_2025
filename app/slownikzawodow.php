<?php
    ...
    // Przygotowujemy dynamiczną klauzulę WHERE
    $where_clause = "";
    $params = [];
    if (!empty($search_term)) {
        // Wyszukujemy po nazwie zawodu, branży lub nazwie umiejętności
        $where_clause = " WHERE LOWER(j.name) LIKE ? OR LOWER(j.industry) LIKE ? OR j.job_id IN (
                            SELECT js.job_id FROM jobs_skills js JOIN skills_1 s ON js.skill_id = s.skill_id WHERE LOWER(s.name) LIKE ?
                        )";
        $params = [$search_param, $search_param, $search_param];
    }

    // Zapytanie liczące wszystkie pasujące zawody
    $sql_count = "SELECT COUNT(DISTINCT j.job_id) AS total FROM jobs j $where_clause";
    ...
    $strona = max(1, min($strona, $total_pages > 0 ? $total_pages : 1));
    $offset = ($strona - 1) * $limit_na_strone;

    // Główne zapytanie pobierające zawody na bieżącą stronę
    $sql_main = "SELECT j.job_id, j.name, j.industry, j.avg_salary FROM jobs j $where_clause ORDER BY j.name ASC LIMIT ? OFFSET ?";
    ...
    $stmt_main->execute();
    $result_jobs = $stmt_main->get_result();
    
    // Tablica do przechowywania zawodów i ich umiejętności
    $zawody = [];
    while ($row = $result_jobs->fetch_assoc()) {
        $zawody[$row['job_id']] = $row;
        $zawody[$row['job_id']]['skills'] = []; // Przygotowujemy miejsce na umiejętności
    }
    $stmt_main->close();

    // Jeśli znaleziono zawody, pobieramy dla nich umiejętności jednym zapytaniem
    if (!empty($zawody)) {
        $job_ids = array_keys($zawody);
        $placeholders = implode(',', array_fill(0, count($job_ids), '?'));
        
        $sql_skills = "SELECT js.job_id, s.name, s.type 
                       FROM jobs_skills js 
                       JOIN skills_1 s ON js.skill_id = s.skill_id 
                       WHERE js.job_id IN ($placeholders)";
        
        $stmt_skills = $conn->prepare($sql_skills);
        $stmt_skills->bind_param(str_repeat('i', count($job_ids)), ...$job_ids);
        $stmt_skills->execute();
        $result_skills = $stmt_skills->get_result();
        
        while ($skill_row = $result_skills->fetch_assoc()) {
            $zawody[$skill_row['job_id']]['skills'][] = $skill_row;
        }
        $stmt_skills->close();
    }
?>
...
<head>
    ...
    <title>KIERUNEKPLOCK.PL - Słownik Zawodów</title>
...
<header class="bg-brand-blue text-white sticky top-0 z-50 shadow-lg">
    ...
    <div class="hidden md:block bg-white"> 
        <div class="max-w-6xl mx-auto px-4 py-3 flex flex-wrap justify-center gap-x-6 gap-y-2 md:gap-x-8">
            ...
            <a href="slownikzawodow.php" class="text-base font-semibold text-brand-blue border-b-2 border-brand-blue">SŁOWNIK ZAWODÓW</a>
    ...
    <div id="mobile-menu" class="md:hidden fixed inset-0 bg-white z-[100] transform translate-x-full transition-transform duration-300">
        <div class="flex flex-col h-full">
            ...
            <div class="flex flex-col space-y-5 p-6 mt-10 text-center">
                ...
                <a href="slownikzawodow.php" class="text-lg font-semibold text-brand-blue border-b-2 border-brand-blue pb-1">SŁOWNIK ZAWODÓW</a>
...
<main class="max-w-5xl mx-auto p-6 mt-10 w-full animate-fadeIn">
    <div class="text-center pb-8 mb-8 border-b border-gray-200">
        <h1 class="text-4xl font-extrabold text-gray-900">Słownik Zawodów</h1>
        <p class="text-lg text-gray-600 mt-2 max-w-2xl mx-auto">Odkryj zawody dostępne na płockim rynku pracy i dowiedz się, jakie umiejętności są kluczowe do ich wykonywania.</p>
    </div>

    <form method="GET" action="slownikzawodow.php" class="mb-8 max-w-3xl mx-auto">
        <div class="flex shadow-sm rounded-lg">
            <input type="text" name="q" placeholder="Wpisz zawód, branżę lub umiejętność..." 
    ...
    <div id="zawody" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php if (!empty($zawody)): ?>
            <?php foreach ($zawody as $zawod): ?>
                <div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between h-full">
                    <div>
                        <h3 class="text-2xl font-bold text-brand-blue mb-2"><?php echo htmlspecialchars($zawod['name']); ?></h3>
                        <p class="text-md text-gray-600 mb-3">Branża: <span class="font-semibold"><?php echo htmlspecialchars($zawod['industry']); ?></span></p>
                        <p class="text-lg font-bold text-green-600 mb-4">Średnie zarobki: <?php echo number_format($zawod['avg_salary'], 0, ',', ' '); ?> PLN</p>
                        
                        <div class="border-t pt-4">
                            <h4 class="font-bold text-gray-700 mb-2">Wymagane umiejętności:</h4>
                            <?php 
                                // Użycie funkcji anonimowej do filtrowania (wymaga PHP 5.3+, a PHP 7+ jest standardem)
                                $hard_skills = array_filter($zawod['skills'], fn($s) => $s['type'] == 'Hard');
                                $soft_skills = array_filter($zawod['skills'], fn($s) => $s['type'] == 'Soft');
                            ?>
                            <?php if (!empty($hard_skills)): ?>
                                <p class="text-sm font-semibold text-gray-500">Twarde:</p>
                                <div class="flex flex-wrap gap-2 mt-1 mb-3">
                                    <?php foreach ($hard_skills as $skill): ?>
                                        <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full"><?php echo htmlspecialchars($skill['name']); ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($soft_skills)): ?>
                                <p class="text-sm font-semibold text-gray-500">Miękkie:</p>
                                <div class="flex flex-wrap gap-2 mt-1">
                                    <?php foreach ($soft_skills as $skill): ?>
                                        <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full"><?php echo htmlspecialchars($skill['name']); ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                            <?php if (empty($hard_skills) && empty($soft_skills)): ?>
                                <p class="text-sm text-gray-500">Brak zdefiniowanych umiejętności dla tego zawodu.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                    <a href="ofertypracy.php?q=<?php echo urlencode($zawod['name']); ?>" class="block w-full mt-4 bg-brand-blue text-white text-center py-2 px-4 rounded-lg font-semibold hover:opacity-90 transition-opacity">Zobacz oferty pracy</a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <p class='text-gray-600 col-span-full text-center text-lg'>Nie znaleziono żadnych zawodów pasujących do Twojego zapytania.</p>
        <?php endif; ?>
        <?php $conn->close(); ?>
    </div>
