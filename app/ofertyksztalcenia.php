<?php
    ...
    $sql_main = "SELECT education_id, name, type, duration_months, required_input 
            FROM education 
            $where_clause
            ORDER BY name
            LIMIT ? OFFSET ?;";
...
<head>
    ...
    <title>KIERUNEKPLOCK.PL - Oferty Edukacji</title>
...
<header class="bg-brand-blue text-white sticky top-0 z-50 shadow-lg">
    ...
    <div class="hidden md:block bg-white"> 
        <div class="max-w-6xl mx-auto px-4 py-3 flex flex-wrap justify-center gap-x-6 gap-y-2 md:gap-x-8">
            ...
            <a href="ofertyksztalcenia.php" class="text-base font-semibold text-brand-blue border-b-2 border-brand-blue">OFERTY EDUKACJI</a>
    ...
    <div id="mobile-menu" class="md:hidden fixed inset-0 bg-white z-[100] transform translate-x-full transition-transform duration-300">
        <div class="flex flex-col h-full">
            ...
            <div class="flex flex-col space-y-5 p-6 mt-10 text-center">
                ...
                <a href="ofertyksztalcenia.php" class="text-lg font-semibold text-brand-blue border-b-2 border-brand-blue pb-1">OFERTY EDUKACJI</a>
    ...
    <main class="max-w-5xl mx-auto p-6 mt-10 w-full animate-fadeIn">
        <div class="text-center pb-8 mb-8 border-b border-gray-200">
            <h1 class="text-4xl font-extrabold text-gray-900">Baza Ofert Edukacyjnych</h1>
            <p class="text-lg text-gray-600 mt-2 max-w-2xl mx-auto">Znajdź studia, kursy i szkolenia w Płocku, które pomogą Ci zdobyć wymarzony zawód.</p>
        </div>

        <form method="GET" action="ofertyksztalcenia.php" class="mb-8 max-w-3xl mx-auto">
            <div class="flex shadow-sm rounded-lg">
                <input type="text" name="q" placeholder="Wpisz nazwę kursu, typ, wymagania..." 
            ...
            <?php
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<div>';
                        echo '<span class="inline-block bg-gray-200 text-brand-blue text-xs font-semibold px-2.5 py-0.5 rounded-full uppercase mb-2">' . htmlspecialchars($row['type']) . '</span>';
                        echo '<h3 class="text-2xl font-bold text-brand-blue mb-2">' . htmlspecialchars($row['name']) . '</h3>';
                        ...
                        echo '</div>';
                        echo '<a href="szczegoly_edukacji.php?education_id=' . $row['education_id'] . '" class="inline-block mt-4 bg-brand-blue text-white text-center py-2 px-5 rounded-lg font-semibold hover:opacity-90 transition-opacity">Szczegóły</a>';
                        echo '</div>';
                    }
                } else {
